...
static bool MakeUserDirs()
{
	...
	if (GetConfigPath(path, sizeof(path), "chew-broadcaster/basic") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "chew-broadcaster/logs") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "chew-broadcaster/profiler_data") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

#ifdef _WIN32
	if (GetConfigPath(path, sizeof(path), "chew-broadcaster/crashes") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
#endif
	if (GetConfigPath(path, sizeof(path), "chew-broadcaster/plugin_config") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
...
static bool MakeUserProfileDirs()
{
	...
	if (GetConfigPath(path, sizeof(path), "chew-broadcaster/basic/profiles") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "chew-broadcaster/basic/scenes") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
...
bool OBSApp::SetTheme(std::string name, std::string path)
{
	...
	if (path == "") {
		...
		string temp = "chew-broadcaster/" + name;
...
static void move_basic_to_profiles(void)
{
	...
	if (GetConfigPath(path, 512, "chew-broadcaster/basic") <= 0)
		return;
	if (!os_file_exists(path))
		return;

	/* if the profiles directory doesn't already exist */
	if (GetConfigPath(new_path, 512, "chew-broadcaster/basic/profiles") <= 0)
		return;
...
static void move_basic_to_scene_collections(void)
{
	...
	if (GetConfigPath(new_path, 512, "chew-broadcaster/basic/scenes") <= 0)
		return;
	if (os_file_exists(new_path))
		return;
...
{
	BPtr<char>       logDir(GetConfigPathPtr("chew-broadcaster/logs"));
	struct os_dirent *entry;
	os_dir_t         *dir        = os_opendir(logDir);
...
static void create_log_file(fstream &logFile)
{
	...
	dst << "chew-broadcaster/logs/" << currentLogFile.c_str();
	...
	if (logFile.is_open()) {
		delete_oldest_file("chew-broadcaster/logs");
...
#define LITERAL_SIZE(x) x, (sizeof(x) - 1)
	ostringstream dst;
	dst.write(LITERAL_SIZE("chew-broadcaster/profiler_data/"));
...
static int run_program(fstream &logFile, int argc, char *argv[])
{
	...
	try {
		...
		delete_oldest_file("chew-broadcaster/profiler_data");
...
#define CRASH_MESSAGE \
	...
	"%appdata%\\chew-broadcaster\\crashes directory)"
...
static void main_crash_handler(const char *format, va_list args, void *param)
{
	...
	delete_oldest_file("chew-broadcaster/crashes");

	string name = "chew-broadcaster/crashes/Crash ";
...
static void move_to_xdg(void)
{
	...
	if (snprintf(old_path, 512, "%s/.chew-broadcaster", home) <= 0)
		return;
	...
	if (GetConfigPath(new_path, 512, "chew-broadcaster") <= 0)
		return;
...
{
	char path[512];
	int pathlen = GetConfigPath(path, 512, "chew-broadcaster/basic/profiles");
